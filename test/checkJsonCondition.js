const { checkJsonCondition } = require("..");
...
result = result && checkJsonCondition(ObjToCheck, Condition) === false;

Condition = { "a.b.c": { $like: "/^someS/i" } }; // Checks that a.b.c meets regex condition
result = result && checkJsonCondition(ObjToCheck, Condition) === true;
...
result = result && checkJsonCondition(3.5, Condition) === true;

Condition = { $: { $gt: 2 } }; // Checks whether the value is greater than 2
result = result && checkJsonCondition(1, Condition) === false;

Condition = { $: { $gte: 7, $lt: 3 } }; // Checks whether the value is greater or equal that 7 and less than 3
result = result && checkJsonCondition(50, Condition) === false;

if (!result) console.error("checkJsonCondition() failed!!!");
